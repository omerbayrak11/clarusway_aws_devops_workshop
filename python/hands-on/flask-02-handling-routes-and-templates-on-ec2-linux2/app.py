...
@app.route("/admin")   # BURDA BASKA BIR PATHE YONLEDIRIYORUZ
def admin():
    return redirect(url_for('error'))
# Create a function named greet which return formatted inline html string 
# and assign to the dynamic route of ('/<name>') 2 kisimda yapilabilir.

#@app.route('/<name>')
#def greet(name): #DINAMIK OLARAK DEGISEN IFADE FONKSIYONLARIN ICINE YAZILIR
   # return f'hello,{name}'
...
@app.route('/<name>')
def greet(name):
    number1 = 34 #iki tane number1 defiskenini ayni tamplete icinde kullanilmaz disarda tanimlanarak kullanilabilinir.
    number2 = 23
    return render_template('greet.html',isim=name,number1=21,number2=34,number3=44,number4=66, toplam = number1 +number2) #
